...
1. De knop "🔐 Inloggen met Google" in de header vraagt via Google Identity Services alleen de `email`-scope op (geverifieerd e-mailadres) — foto's toevoegen gaat inmiddels via een bestandskiezer van het toestel zelf, niet meer via Google, dus daar is geen aparte scope meer voor nodig.
